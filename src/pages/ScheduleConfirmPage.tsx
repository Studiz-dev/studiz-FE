...
import ScheduleConfirmSuccessModal from "../components/ScheduleConfirmSuccessModal";
  ...
  const handleConfirm = () => {
    ...
    if (cell) {
      console.log("일정 확정:", {
        ...
        time: `${String(selectedCell.timeSlot.hour).padStart(2, "0")}:${String(selectedCell.timeSlot.minute).padStart(2, "0")}`,
        availableMembers: cell.availableMembers?.length || 0,
      });
      // TODO: 일정 확정 API 호출
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="relative flex items-center justify-center h-[60px] bg-white border-b-[1.5px] border-main4 px-4">
        ...
        <h1 className="text-lg font-semibold text-black1">시간표 확인</h1>
      </div>

      <div className="flex-1 overflow-y-auto pt-4 pb-24 relative">
        {/* 시간 등록 현황 */}
        <div className="mb-4 px-4">
          <div className="text-[18px] font-semibold text-black1 mt-2 mb-2">
            시간 등록 현황
          </div>
        ...
        <div className="overflow-x-auto relative mt-8 pl-4">
          <div className="inline-block">
            <table className="border-collapse">
              ...
              <thead>
                <tr className="h-6">
                  <th className="w-16 h-6 bg-white"></th>
                  {scheduleData.dates.map((date) => (
                    <th
                      key={date}
                      className="text-center text-[12px] font-medium py-1 bg-white align-middle h-6"
                    >
            ...
            return (
              <div className="bg-white p-4 relative">
                ...
                <div className="flex items-center justify-between mb-2 mt-1">
                  ...
                  <div className="text-[18px] font-semibold text-black1 text-center flex-1">
                  ...
                  <div className="flex-1 flex justify-center">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-[15px] text-sm font-medium transition whitespace-nowrap ${
                        selectedCell
                          ? "bg-point text-white hover:bg-[#4C6953]"
      ...
      {selectedCell && (
        <ScheduleConfirmModal
          ...
          studyName={scheduleData.studyName}
          date={(() => {
            const dateParts = selectedCell.date.split(" ");
            const dateNum = dateParts[0].split("/")[0];
            const dayOfWeek = dateParts[1];
            return `2025년 9월 ${dateNum}일 (${dayOfWeek})`;
          })()}
          time={`${String(selectedCell.timeSlot.hour).padStart(2, "0")}:${String(selectedCell.timeSlot.minute).padStart(2, "0")}`}
        />
      )}
